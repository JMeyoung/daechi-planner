...
function fmtTimeInput(s: string) {
  const d = new Date(s)
...
export default function EditSchedulePage() {
  ...
  const [form, setForm] = useState({
    title: '',
    category: 'academy' as ScheduleCategory,
    subject: '',
    location: '',
    date: '',
    startTime: '16:00',
  ...
  return (
    <div className="max-w-lg pb-10">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-lg font-bold text-gray-900">일정 편집</h1>
        <button
          type="button"
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-5">
        ...
        <div>
          ...
          <input type="text" value={form.subject} onChange={e => set('subject', e.target.value)} placeholder="수학, 영어 등" className={input} />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">장소</label>
          <input type="text" value={form.location} onChange={e => set('location', e.target.value)} placeholder="대치동 OO학원" className={input} />
        </div>
